...
The card is extremely versetile and can be used on its own, or as a building block for [accordions](/components/accordion), [dialogs](/components/dialog) and more.
...
The [accordion group](/components/accordion#accordion-group) is a great example where Open Props UI leverages the text variant of the `.card` component.
...
[Accordion](/components/accordion), [Dialog](/components/dialog)
...
::: code-group
<<< @/../src/themes/theme-one/components/card.css [card.css]
:::
